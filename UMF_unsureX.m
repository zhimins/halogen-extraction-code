function [list_UMF]=UMF_unsureX(list_MS,foldername)
...
for i=1:size(list_UMF,1)
    ...
    count=1;
    for j=2:size(raw,1)
        if(raw{j,16}+raw{j,18}+raw{j,19}+raw{j,20}>0)
            count=count+1;
    ...
    for j=size(raw_right,1):-1:2
        for k=size(raw_select,1):-1:2
            if(strcmp(raw_select{k,3},raw_right{j,3}))
                raw_select(k,:)=[];
            end
        end
    end
    for j=size(raw_select,1):-1:2
        for k=size(raw,1):-1:2
            if(strcmp(raw{k,3},raw_select{j,3}))
                raw(k,:)=[];
            end
        end
    end
